...
set constr_zybo_timing       "$constr_path/constr_zybo_timing.xdc"
set constr_zybo_ip		     "$constr_path/constr_zybo_ip.xdc"
set constr_zybo_top		     "$constr_path/constr_zybo_top.xdc"

set constr_zybo_list	     "$constr_zybo_timing \
							  $constr_zybo_ip \
							  $constr_zybo_top"
...
# 3) repeating 1) for harmonia IPs
if { [string  equal [glob -directory $ip_infineon_path -types d -nocomplain harmonia*] ""]} {
	puts "Harmonia IPs are not yet unpacked. Unpacking.."
	foreach i $ip_infineon_names { update_ip_catalog -add_ip $i -repo_path $ip_infineon_path }
} else {
	#report what IPs in ip_path has been found
	set unpacked_harmonia_ip_folders [glob -directory $ip_infineon_path -types d harmonia*]
	set unpacked_harmonia_ip_names [lsearch -regexp -all -inline [get_ipdefs] "harmonia*"]
	foreach i $unpacked_harmonia_ip_folders { puts "IP folders found:\t [file tail $i]" }
	foreach i $unpacked_harmonia_ip_names { puts "IP found:\t [file tail $i]" }
}
...
foreach constr_file $constr_zybo_list {
...
set_property "target_constrs_file" $constr_zybo_top $obj

puts "************************************************************************"
puts "Creating block designs ..."
puts "************************************************************************"

#creating block design of the system to debug
source $src_path/bd/ip_debug_bd.tcl


##############################################################
#setting top object
set top_obj  ip_debug_bd_wrapper
...
set files [list \
 "[file normalize "$src_path/sim/input_gen_sim.vhd"]"\
 "[file normalize "$src_path/sim/ip_debug_sim.vhd"]"\
]
add_files -norecurse -fileset $obj $files

##############################################################
#setting top simulation object
set top_sim_file       ip_debug_sim
